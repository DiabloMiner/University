...
def evc_gamma_correct(input_image: np.ndarray, gamma: float) -> np.ndarray:
    ...
    clipped_gamma = max(1.0 / float(1 << 63), gamma)
    corrected = np.pow(input_image, 1.0 / clipped_gamma)
    ### END STUDENT CODE
...
def evc_reconstruct(brightness_corrected: np.ndarray, chromaticity) -> np.ndarray:
    ...
    R = brightness_corrected * chromaticity[:,:,0]
    G = brightness_corrected * chromaticity[:,:,1]
    B = brightness_corrected * chromaticity[:,:,2]
