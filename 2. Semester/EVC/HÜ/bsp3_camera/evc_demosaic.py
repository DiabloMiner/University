...
def evc_interpolate(red : np.ndarray, green : np.ndarray, blue : np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
	...
	# NOTE: The following three lines can be removed. They prevent the framework
    #       from crashing.
    filterG = np.array([[0.0, 0.25, 0.0],
                       [0.25, 1.0, 0.25],
                       [0.0, 0.25, 0.0]])
    filterBR = np.array([[0.25, 0.5, 0.25],
                       [0.5, 1.0, 0.5],
                       [0.25, 0.5, 0.25]])

    R_inter = scipy.ndimage.correlate(red, filterBR, mode='constant')
    G_inter = scipy.ndimage.correlate(green, filterG, mode='constant')
    B_inter = scipy.ndimage.correlate(blue, filterBR, mode='constant')
    ### END STUDENT CODE
...
def evc_concat(R: np.ndarray, G: np.ndarray, B: np.ndarray) -> np.ndarray:
    ...
    result = np.stack(arrays=[R, G, B], axis=2)
    # TODO: Fix juypter notebook error
    ### END STUDENT CODE
